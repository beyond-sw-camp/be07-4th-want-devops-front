        ...
        <v-row>
            ...
            <v-col cols="12" md="4" class="block-list" ref="blockList">
                ...
                <v-list>
                    ...
                    >
                    {{ block.title }}
                    <!-- 좋아요 눌린 블럭은 하트가 눌려있도록.. 수정 중 -->
                    <!-- Heart Entity 사용하기 -->
                    <v-icon
                    :class="{ 'liked': block.liked }"
                    @click.stop="toggleLike(block)"
                    :style="{ color: block.liked ? 'red' : 'inherit' }"
                    >
                    mdi-heart
                    </v-icon>
                    <!-- 좋아요 수 확인 -->
...
export default {
    ...
    methods: {
        async fetchData() {
            ...
            try {
                // 토큰으로 프로젝트에 소속된 멤버임을 확인하기
                // 테스트를 위해 프로젝트 아이디를 1로 고정함.
                const projectId = 1;
        ...
        filterByCategory(category) {
            if (this.selectedCategory === category) {
                // 이미 선택된 카테고리를 다시 클릭했을 때 모든 블록을 보여주기 위해 null로 설정
                this.selectedCategory = null;
            } else {
                // 다른 카테고리를 클릭했을 때 해당 카테고리만 보여주도록 설정
